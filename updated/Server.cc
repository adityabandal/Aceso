...
Server::Server(uint8_t server_id, bool is_recovery, uint32_t ckpt_interval, uint32_t block_size) {
    ...
    if (!is_recovery) {
        ...
        printf("FINSHI CREATE NETWORK MANAGER");
        _init_hashtable();
        _init_block();
        _init_ckpt();
    } else {
...
void * Server::thread_main() {
    int rc = 0;
    //printf("Thread run main ...\n");
    stick_this_thread_to_core(define::mainCoreID);
    //printf("Continue Thread run main ...\n");
    ...
    while (!need_stop) {
        ...
        // printf("opcode      : %d\n", wc.opcode);
        // printf("qp_num      : 0x%x\n", wc.qp_num);
        // printf("src_qp      : 0x%x\n", wc.src_qp);   // Only valid in UD
        // printf("wc_flags    : 0x%x\n", wc.wc_flags);
        // printf("byte_len    : %u\n", wc.byte_len);
        // printf("imm_data    : %u\n", wc.imm_data);   // if wc_flags has IBV_WC_WITH_IMM
        // printf("slid        : %u\n", wc.slid);       // Only valid in UD
        // printf("sl          : %u\n=====================\n", wc.sl);         // Only valid in UD
        /*
	printf("==== Received message ====\n");
        printf("Local QP Number (qp_num)   : 0x%x\n", wc.qp_num);
        printf("Source QP (src_qp, UD only): 0x%x\n", wc.src_qp);
        printf("Source LID (slid, UD only) : 0x%x\n", wc.slid);
