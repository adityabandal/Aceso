...
int stick_this_thread_to_core(int core_id) {
    //printf("stick_this_thread_to_core %d\n", core_id);fflush(stdout);
    int num_cores = sysconf(_SC_NPROCESSORS_CONF);
    if (core_id < 0 || core_id >= num_cores) {
        // printf("thread bind core error\n");
        return -1;
    }
    // printf("stick_this_thread_to_core\n");fflush(stdout);
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    ...
    for (int i = 0; i < sysconf(_SC_NPROCESSORS_CONF); i ++) {
        ...
        // printf("i = %d, sysconf = %d, stick_this_thread_to_core\n", i, sysconf(_SC_NPROCESSORS_CONF));fflush(stdout);
    }
    //printf("FINISH stick\n");fflush(stdout);
    return 0;
}
